...
export const AUDIT_CATEGORY_ORDER: AuditCategory[] = AUDIT_CATEGORIES.map(
  (category) => category.key,
)

export const getAuditCategoryMeta = (category: AuditCategory): AuditCategoryMeta =>
  AUDIT_CATEGORIES.find((item) => item.key === category) ?? AUDIT_CATEGORIES[0]
